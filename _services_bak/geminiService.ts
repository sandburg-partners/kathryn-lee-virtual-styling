...
export const generateComposite = async (referenceSheet: ImageData, inputs: TryOnInputs): Promise<CompositeResult> => {
  ...
  const response = await ai.models.generateContent({
      ...
      config: {
          // FIX: The 'responseModalities' for image generation must be an array with a single `Modality.IMAGE` element.
...
export const refineComposite = async (previousResult: CompositeResult, prompt: string): Promise<CompositeResult> => {
  ...
  const response = await ai.models.generateContent({
      // FIX: Updated model name to 'gemini-2.5-flash-image' as per guidelines for image editing.
      model: 'gemini-2.5-flash-image',
      contents: { parts: [previousImagePart, textPart] },
      config: {
          // FIX: The 'responseModalities' for image editing must be an array with a single `Modality.IMAGE` element.
          responseModalities: [Modality.IMAGE],
      },
  });
